...
int main (int argc, char *argv[])
{
    ...
    if(cuda_ret != cudaSuccess) { printf("Unable to allocate A_d\n"); exit(-1); }

    cuda_ret = cudaMalloc((void**)&B_d, sizeof(float) * B_sz);
    if(cuda_ret != cudaSuccess) { printf("Unable to allocate B_d\n"); exit(-1); }

    cuda_ret = cudaMalloc((void**)&C_d, sizeof(float) * C_sz);
    if(cuda_ret != cudaSuccess) { printf("Unable to allocate C_d\n"); exit(-1); }
    ...
    if(cuda_ret != cudaSuccess) { printf("Unable to copy A_h to A_d\n"); exit(-1); }

    cuda_ret = cudaMemcpy(B_d, B_h, sizeof(float) * B_sz, cudaMemcpyHostToDevice);
    if(cuda_ret != cudaSuccess) { printf("Unable to copy B_h to B_d\n"); exit(-1); }
    ...
    if(cuda_ret != cudaSuccess) { printf("Unable to copy C_d to C_h\n"); exit(-1); }
